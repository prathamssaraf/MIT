...
def load_data(directory):
    ...
    """
    # Load people
    with open(f"degrees\large\people.csv", encoding="utf-8") as f:
    ...
    # Load moviesJohn Belushi
    with open(f"degrees\large\movies.csv", encoding="utf-8") as f:
        reader = csv.DictReader(f)
        for row in reader:
            movies[row["id"]] = {
                "title": row["title"],
    ...
    # Load stars
    with open(f"degrees\large\stars.csv", encoding="utf-8") as f:
        reader = csv.DictReader(f)
        for row in reader:
            try:
...
def shortest_path_dfs(source, target):
    ...
    while True:
        ...
        if frontier.empty():
            print('Frontier is Empty - No Connection Between Actors!')
            print(len(explored), 'actors explored with no solution found!')
            return None

        # Otherwise expand the next node in the stack, add it to the explored set and get set of movies and actors for the actor in the current node:
        curr_node = frontier.remove()
        explored.add(curr_node.state)
        ...
        for action, state in neighbors_for_person(curr_node.state):
            ...
            # Otherwise add the new states to explore to the stack:
            if not frontier.contains_state(state) and state not in explored:
                new_node = Node(state, curr_node, action)
                frontier.add(new_node)
